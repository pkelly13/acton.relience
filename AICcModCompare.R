...
auto.mod<-lm(d$chl~d$chl.tminus1)
#full model
full.mod<-lm(d$chl~d$chl.tminus1+d$logshad.p+d$lognvss+d$srp.load+d$logzoops)
shadNVSSsrp<-lm(d$chl~d$chl.tminus1+d$logshad.p+d$lognvss+d$srp.load)
shadNVSS<-lm(d$chl~d$chl.tminus1+d$logshad.p+d$lognvss)
shad<-lm(d$chl~d$chl.tminus1+d$logshad.p)

nvss<-lm(d$chl~d$chl.tminus1+d$lognvss)
mods<-list()
mods[[1]]<-auto.mod
mods[[2]]<-shad
mods[[3]]<-shadNVSS
...
aictab(cand.set=mods,modnames=c('auto','shad only','shad+NVSS','shad+NVSS+srp','shad+NVSS+srp+zoops'))
